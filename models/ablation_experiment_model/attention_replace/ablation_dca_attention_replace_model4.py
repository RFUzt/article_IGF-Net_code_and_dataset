...
class CrossAttentionBlock(nn.Module):
    ...
    def forward(self, x, y):
        ...
        k = self.k_proj(y).view(B, L, self.num_heads, self.d_k).transpose(1,2)
        v = self.v_proj(y).view(B, L, self.num_heads, self.d_k).transpose(1,2)
        scale = self.d_k ** -0.5
        out = self.attn(q, k, v, scale)
        out = out.transpose(1,2).contiguous().view(B, L, D)
        out = self.out_proj(out)
        return out

class DCA_Fuse(nn.Module):
    def __init__(self, channels, num_heads=4):
        super().__init__()
        self.channels = channels
        self.num_heads = num_heads
        self.ch_cross = CrossAttentionBlock(channels, num_heads)
        self.sp_cross = CrossAttentionBlock(channels, num_heads)
    def forward(self, x, y):
        B, C, H, W = x.shape
        L = H * W
        x_flat = x.view(B, C, L).permute(0, 2, 1)  # [B, L, C]
        y_flat = y.view(B, C, L).permute(0, 2, 1)
        out1 = self.ch_cross(x_flat, y_flat)
        out2 = self.sp_cross(out1, y_flat)
...
class AblationModel4(nn.Module):
    def __init__(self, in_channels=14, num_classes=1, apply_sigmoid=True):
        ...
        self.assemble_layer = DCA_Fuse(channels=256)
...
@register_model("ablation4")
def build_ablation4(in_channels=3, num_classes=1, apply_sigmoid=True, **kwargs):
    return AblationModel4(in_channels=in_channels, num_classes=num_classes, apply_sigmoid=apply_sigmoid, **kwargs)


if __name__ == "__main__":
    model = AblationModel4(in_channels=5, num_classes=1)
    summary(model, input_size=(5, 128, 128), device="cpu")
