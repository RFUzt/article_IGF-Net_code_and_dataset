# paper：Bilinear Attention Networks
...
class BilinearAttentionFusion(nn.Module):
    def __init__(self, channels, rank=32):
        ...
        self.left_proj = nn.Linear(channels, rank, bias=False)
        self.right_proj = nn.Linear(channels, rank, bias=False)
        self.att_proj = nn.Linear(rank, channels, bias=False)
    ...
    def forward(self, x, y):
        ...
        x_flat = x.view(B, C, L).permute(0, 2, 1)
        y_flat = y.view(B, C, L).permute(0, 2, 1)

        x_low = self.left_proj(x_flat)
        y_low = self.right_proj(y_flat)

        att_score = (x_low * y_low).sum(dim=-1, keepdim=True)

        att_weight = torch.sigmoid(att_score)

        att_weight = att_weight.expand(-1, -1, C)

        fused_flat = x_flat * att_weight + y_flat * (1 - att_weight)
...
class AblationModel2(nn.Module):
    def __init__(self, in_channels=14, num_classes=1, apply_sigmoid=True):
        ...
        self.assemble_layer = BilinearAttentionFusion(channels=256)
...
@register_model("ablation2")
def build_ablation2(in_channels=3, num_classes=1, apply_sigmoid=True, **kwargs):
    return AblationModel2(in_channels=in_channels, num_classes=num_classes, apply_sigmoid=apply_sigmoid, **kwargs)


if __name__ == "__main__":
    model = AblationModel2(in_channels=14, num_classes=1)
