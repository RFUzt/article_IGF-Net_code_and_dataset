# paper：Cross-modality attentive feature fusion for object detection in multispectral remote sensing imagery
...
class CMAFF(nn.Module):
    ...
    def __init__(self, channels, reduction=4):
        ...
        self.common_att = nn.Sequential(
            nn.Conv2d(channels * 2, inter, kernel_size=1, bias=False),
    ...
    def forward(self, x, y):
        ...
        w_c = self.common_att(xy)
        w_d = self.diff_att(xy)

        f_c = 0.5 * (x + y)
        f_d = x - y

        out = x * w_c + y * (1 - w_c) + f_d * w_d

        out2 = self.proj(torch.cat([x, y], dim=1))
        return out + out2
...
class AblationModel3(nn.Module):
    def __init__(self, in_channels=14, num_classes=1, apply_sigmoid=True):
        ...
        self.assemble_layer = CMAFF(channels=256)
...
@register_model("ablation3")
def build_ablation3(in_channels=3, num_classes=1, apply_sigmoid=True, **kwargs):
    return AblationModel3(in_channels=in_channels, num_classes=num_classes, apply_sigmoid=apply_sigmoid, **kwargs)


if __name__ == "__main__":
    model = AblationModel3(in_channels=5, num_classes=1)
