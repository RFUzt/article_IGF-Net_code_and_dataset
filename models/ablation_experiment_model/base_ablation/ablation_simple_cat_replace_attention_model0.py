...
class AblationModel0(nn.Module):
    def __init__(self, in_channels=14, num_classes=1, apply_sigmoid=True):
        ...
        self.mult_encoder1 = nn.Sequential(
            *list(mult_base_encoder.children())[0:3])
        ...
        self.assemble_layer = nn.Sequential(
    ...
    def forward(self, x):
        ...
        assemble1 = torch.cat((rgb_2, mult2), dim=1)
        low_level = self.assemble_layer(assemble1)
...
@register_model("ablation0")
def build_ablation0(in_channels=3, num_classes=1, apply_sigmoid=True, **kwargs):
    return AblationModel0(in_channels=in_channels, num_classes=num_classes, apply_sigmoid=apply_sigmoid, **kwargs)


if __name__ == "__main__":
    model = AblationModel0(in_channels=14, num_classes=1)
